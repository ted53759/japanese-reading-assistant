...
`web/` 提供 Vue 3 + Vite 操作介面，讓使用者可以：

- 選擇日文 TXT，先預覽切句結果
- 使用自己的 OpenAI API Key，以每批最多 20 句逐批處理整個章節
- 同時顯示整份文件與目前批次的逐句進度，並可在完成目前句子後暫停
- 保留全域句子編號，並將多批結果合併下載為 TXT／JSON
- 查看原句、逐字拆解、假名、繁中解釋與翻譯
- 查看遺漏編號、重複編號與格式錯誤等品質檢核
- 下載 UTF-8 TXT 或 JSON 結果

API Key 只會送到 `127.0.0.1` 的本機 Python 服務，僅在當次請求的記憶體中使用，不會寫入檔案。

安裝與啟動方式請見 [`web/README.md`](web/README.md)。
